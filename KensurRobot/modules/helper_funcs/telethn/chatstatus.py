from KensurRobot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from KensurRobot import SUDO_USERS
...
async def KensurRobot_is_admin(chat_id: int):
    status = False
    KensurRobot = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if KensurRobot.id == user.id:
            status = True
            break
    return status
